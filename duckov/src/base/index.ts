import { BaseConstructionInterface, FacilityStatus, Facility, Base, LootItem, Resource, Player } from '../shared/types';

export class BaseConstruction implements BaseConstructionInterface {
  private base: Base;
  private player: Player;
  private facilitiesData: Map<string, {
    name: string;
    type: Facility['type'];
    maxLevel: number;
    baseCost: Map<string, Resource>;
    upgradeCostMultiplier: number;
    effects: Map<number, Facility['effects']>;
  }>;

  constructor(base: Base, player: Player) {
    this.base = base;
    this.player = player;
    this.facilitiesData = this.initializeFacilitiesData();
  }

  private initializeFacilitiesData(): Map<string, any> {
    return new Map([
      ['workbench', {
        name: '工作台',
        ...
        maxLevel: 5,
        baseCost: new Map([
          ['metal', { id: 'metal', name: '金属', type: 'metal', amount: 100 }],
          ['wood', { id: 'wood', name: '木材', type: 'wood', amount: 50 }]
        ]),
      ...
      ['research-station', {
        name: '蓝图研究站',
        type: 'research-station' as const,
        maxLevel: 5,
        baseCost: new Map([
          ['metal', { id: 'metal', name: '金属', type: 'metal', amount: 150 }],
          ['electronic', { id: 'electronic', name: '电子元件', type: 'electronic', amount: 50 }]
        ]),
        upgradeCostMultiplier: 1.5,
        effects: new Map([
      ...
      ['medical-station', {
        name: '医疗站',
        type: 'medical-station' as const,
        maxLevel: 5,
        baseCost: new Map([
          ['metal', { id: 'metal', name: '金属', type: 'metal', amount: 100 }],
          ['chemical', { id: 'chemical', name: '化学物质', type: 'chemical', amount: 50 }]
        ]),
        upgradeCostMultiplier: 1.5,
        effects: new Map([
      ...
      ['armory', {
        ...
        maxLevel: 5,
        baseCost: new Map([
          ['metal', { id: 'metal', name: '金属', type: 'metal', amount: 200 }],
          ['wood', { id: 'wood', name: '木材', type: 'wood', amount: 100 }]
        ]),
        upgradeCostMultiplier: 1.5,
        effects: new Map([
      ...
      ['gym', {
        ...
        baseCost: new Map([
          ['metal', { id: 'metal', name: '金属', type: 'metal', amount: 150 }],
          ['wood', { id: 'wood', name: '木材', type: 'wood', amount: 150 }]
        ]),
        upgradeCostMultiplier: 1.5,
        effects: new Map([
  ...
  getFacilityStatus(facilityId: string): FacilityStatus {
    ...
    const facilityData = this.facilitiesData.get(facilityId);
    const nextLevel = facility.level + 1;
    let upgradeCost: Resource[] = [];

    if (facilityData && nextLevel <= facilityData.maxLevel) {
      const upgradeCostMap = this.calculateUpgradeCost(facilityData.baseCost, nextLevel, facilityData.upgradeCostMultiplier);
      upgradeCost = Array.from(upgradeCostMap.values());
    }
  ...
  private useWorkbench(action: string, params: any): any {
    switch (action) {
      case 'craft':
  ...
  private useResearchStation(action: string, params: any): any {
    if (action === 'research') {
  ...
  private useMedicalStation(action: string, params: any): any {
    switch (action) {
      case 'heal':
  ...
  private useArmory(action: string, params: any): any {
    switch (action) {
      case 'store':
  ...
  private useGym(action: string, params: any): any {
    if (action === 'train') {
  ...
  private useStorageLocker(action: string, params: any): any {
    switch (action) {
      case 'store':
  ...
  private craftItem(blueprintId: string, materials: Resource[]): any {
  ...
  private craftMedicine(medicineId: string, materials: Resource[]): any {
  ...
  private storeInArmory(items: LootItem[]): any {
    console.log(`使用军备室存储物品: ${items.length} 件`);
    return { success: true, stored: items.length };
  }

  private retrieveFromArmory(itemIds: string[]): any {
    console.log(`使用军备室取回物品: ${itemIds.length} 件`);
    return { success: true, retrieved: itemIds.length };
  }

  private trainStats(statType: string, amount: number): any {
    if (this.player.stats[statType as keyof typeof this.player.stats]) {
      (this.player.stats[statType as keyof typeof this.player.stats] as number) += amount;
      console.log(`使用健身房训练 ${statType}，增加 ${amount} 点`);
      return { success: true, stat: statType, value: this.player.stats[statType as keyof typeof this.player.stats] };
    }
    return { success: false, message: '无效的属性类型' };
  }

  private storeInLocker(items: LootItem[]): any {
    console.log(`使用储物柜存储物品: ${items.length} 件`);
    return { success: true, stored: items.length };
  }

  private retrieveFromLocker(itemIds: string[]): any {
    console.log(`使用储物柜取回物品: ${itemIds.length} 件`);
    return { success: true, retrieved: itemIds.length };
  }

  private hasEnoughResources(cost: Map<string, Resource>): boolean {
    for (const [resourceId, requiredResource] of cost) {
      const availableResource = this.base.resources.find(r => r.id === resourceId);
  ...
  private consumeResources(cost: Map<string, Resource>): void {
    for (const [resourceId, requiredResource] of cost) {
      const availableResource = this.base.resources.find(r => r.id === resourceId);
      if (availableResource) {
        availableResource.amount -= requiredResource.amount;
      }
    }
  }

  private calculateUpgradeCost(baseCost: Map<string, Resource>, level: number, multiplier: number): Map<string, Resource> {
    const upgradedCost = new Map<string, Resource>();
    for (const [resourceId, resource] of baseCost) {
      upgradedCost.set(resourceId, {
        ...resource,
        amount: Math.floor(resource.amount * Math.pow(multiplier, level - 1))
      });
    }
    return upgradedCost;
  }
